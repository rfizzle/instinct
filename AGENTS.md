...
[`design/VISION.md`](design/VISION.md); the full behavioral spec is
...
- [HUD-STANDARD.md](https://github.com/rfizzle/concord/blob/master/HUD-STANDARD.md) — Instinct has **no HUD slot, by design** (`design/DESIGN.md` §2)
...
|---|---|---|
| `main` | `src/main/java` | Server + common logic. Entrypoint: `Instinct.java` |
| `client` | `src/client/java` | Client-only code. Entrypoint: `InstinctClient.java` |
...
- **Mod ID:** `instinct` — use `Instinct.id("path")` to create
...
- **Architectural stance (SPEC.md intro):** augment, never replace — no
  ...
  `#instinct:*` tags → heuristic) — see SPEC.md "Animal Coverage".
- **Asset philosophy:** vanilla animals keep vanilla looks, always. Custom
  ...
  custom `/sfx` synthesis; organic sounds stay vanilla. Masters live in
  `art/` with `.glyph` sources in `art/glyphs/`.
- **Mixin config:** `instinct.mixins.json` in `src/main/resources`, mixin
  package `com.rfizzle.instinct.mixin` — created with the first mixin.
- **Commits:** [Conventional Commits](https://www.conventionalcommits.org/)
  with a topical scope naming the feature area: `feat(veterancy): …`,
  `fix(trough): …`, `refactor(genetics): …`, `ci(review): …`,
  `docs(readme): …`. Allowed types: `feat`, `fix`, `refactor`, `chore`,
  `docs`, `test`, `build`, `ci`, `perf`, `style`. Subject line in imperative
  mood, no trailing period, ≤72 chars. Reference the issue in the body
  footer: `Closes #42` (or `Refs #42` for partial work).
...
| Path | Purpose |
|---|---|
| `design/VISION.md` | The player-experience promise — the domain gate for incoming work. |
| `design/SPEC.md` | Full behavioral spec for the mod. |
| `design/DESIGN.md` | Brand, palette, asset specs — the pre-implementation "why & what". |
| `site/` | Structured website content (source for [instinct.rfizzle.com](https://instinct.rfizzle.com)), rendered by the shared Concord template. |
