...
The gesture is `shoulderDismountGesture` (default `DOUBLE_TAP_SNEAK`). Players sneak constantly — working a ledge, placing a block precisely, peeking over an edge — so the drop is keyed to a motion players rarely make by accident: **two sneak taps within `shoulderDismountDoubleTapTicks`** (default 12). A tap counts on release, and only when the press was held no longer than that window, so a held crouch contributes nothing; the second tap's press is what drops the bird. `SNEAK` is the alternative — any sneak, tapped or held — for a server that wants the drop on the crouch itself.

One deliberate crouch pattern does read as the gesture: because an inspection line (§2) re-answers once crouch is released and taken again, flicking crouch twice inside the window to re-poll a pet's line is press-release-press, and a perched bird comes off. The bird is standing right there to pick back up, so this is a cost worth the ledge work the gesture buys.

Because the gesture is read from the sneak state the server already tracks, it is sampled once per tick: two taps that both begin and end inside a single tick are not distinguishable, which is what `shoulderDismountDoubleTapTicks` exists to widen. For the same reason the gesture counts state changes, not key presses — with vanilla's Sneak set to Toggle, each press flips the state, so the taps are three presses (on, off, on) rather than two, and `SNEAK` is the single-press option for anyone who wants one.
